...
- une fonction `assertNodeType(node: Node)` qui lève une exception si `node` n'est pas de type _NodeType_ ;
- une fonction `isNodeType(node: Node)` qui renvoie `true` si `node` est de type _NodeType_ ;
